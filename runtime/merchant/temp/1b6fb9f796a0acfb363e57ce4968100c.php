...
<div class="wrapper wrapper-content animated fadeInRight">
    <form class="form-horizontal m" id="hr-form" data-ref="resourceType">
        <input  type="hidden" name="id" value="<?php echo htmlentities((string) $model['id']); ?>"/>
            <div class="form-group">
       <label  class="col-sm-3 control-label is-required" >所属商户</label>
        <div class="col-sm-8">
           <select class="form-control select2" id="merchant_id" data-value="<?php echo htmlentities((string) $model['merchant_id']); ?>" name="merchant_id"   required >
  <option value="">请选择</option>
<?php $_result=get_dict('sys_fans_type');if(is_array($_result) || $_result instanceof \think\Collection || $_result instanceof \think\Paginator): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dict): $mod = ($i % 2 );++$i;?>
...
<div class="radio-box">
    <input type="radio" id="type_name_<?php echo htmlentities((string) $dict['dict_value']); ?>" name="type_name" value="<?php echo htmlentities((string) $dict['dict_value']); ?>" <?php echo $dict['dict_value']==$model['type_name'] ? "checked"  :  ""; ?>>
    <label for="type_name_<?php echo htmlentities((string) $dict['dict_value']); ?>" ><?php echo htmlentities((string) $dict['dict_label']); ?></label>
</div>
<?php endforeach; endif; else: echo "" ;endif; ?>
        </div>
    </div>
