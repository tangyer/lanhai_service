...
<div class="wrapper wrapper-content animated fadeInRight">
    <form class="form-horizontal m" id="hr-form" data-ref="keyword">
        <input  type="hidden" name="id" value="<?php echo htmlentities((string) $model['id']); ?>"/>
            <div class="form-group">
               <label  class="col-sm-3 control-label is-required" >敏感词</label>
                <div class="col-sm-8">
                   <input  type="text" id="keyword" class="form-control " placeholder="请输入敏感词" name="keyword" value="<?php echo htmlentities((string) $model['keyword']); ?>" required />
                </div>
            </div>
            <div class="form-group">
                <label  class="col-sm-3 control-label is-required" >分类</label>
                <div class="col-sm-8">
                    <select class="form-control " name="keyword_type"  data-toggle="select" id="keyword_type" data-url="<?php echo url('keywordType/getAll'); ?>" data-value="<?php echo htmlentities((string) $model['keyword_type']); ?>" data-field="type_name" required></select>
                </div>
            </div>

            <div class="form-group">
               <label  class="col-sm-3 control-label is-required" >拒绝发送</label>
                <div class="col-sm-8">
               <?php $_result=get_dict('sys_yes_no');if(is_array($_result) || $_result instanceof \think\Collection || $_result instanceof \think\Paginator): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dict): $mod = ($i % 2 );++$i;?>
                <div class="radio-box">
                        <input type="radio" id="is_refuse_<?php echo htmlentities((string) $dict['dict_value']); ?>" name="is_refuse" value="<?php echo htmlentities((string) $dict['dict_value']); ?>" <?php echo $dict['dict_value']==$model['is_refuse'] ? "checked"  :  ""; ?>>
                        <label for="is_refuse_<?php echo htmlentities((string) $dict['dict_value']); ?>" ><?php echo htmlentities((string) $dict['dict_label']); ?></label>
                    </div>
