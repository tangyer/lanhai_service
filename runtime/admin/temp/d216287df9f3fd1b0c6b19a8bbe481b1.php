...
<div class="wrapper wrapper-content animated fadeInRight ibox-content">
    <form class="form-horizontal m" id="hr-form" data-ref="merchant">
        <input  type="hidden" name="id" value="<?php echo htmlentities((string) $model['id']); ?>"/>
            <div class="form-group">
       <label  class="col-sm-3 control-label " >商户名称</label>
        <div class="col-sm-8">
           <input  type="text" id="merchant_name" class="form-control " placeholder="请输入商户名称" name="merchant_name" value="<?php echo htmlentities((string) $model['merchant_name']); ?>"  />
        </div>
    </div>
    <div class="form-group">
       <label  class="col-sm-3 control-label " >开通平台</label>
        <div class="col-sm-8">
           <?php $_result=get_dict('sys_trans_platform');if(is_array($_result) || $_result instanceof \think\Collection || $_result instanceof \think\Paginator): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dict): $mod = ($i % 2 );++$i;?>
             <label  class="check-box">
                <input type="checkbox" class="platform_type_<?php echo htmlentities((string) $dict['dict_value']); ?>" id="platform_type_<?php echo htmlentities((string) $dict['dict_value']); ?>" name="platform_type[]" value="<?php echo htmlentities((string) $dict['dict_value']); ?>" >
                <?php echo htmlentities((string) $dict['dict_label']); ?>
            </label>
            <?php endforeach; endif; else: echo "" ;endif; ?>
            <input type="hidden" id="platform_type_input" value="<?php echo htmlentities((string) $model['platform_type']); ?>">
        </div>
    </div>
    <div class="form-group">
       <label  class="col-sm-3 control-label " >端口数量</label>
        <div class="col-sm-8">
           <input  type="text" id="port_num" class="form-control " placeholder="请输入端口数量" name="port_num" value="<?php echo htmlentities((string) $model['port_num']); ?>"  />
        </div>
    </div>
    <div class="form-group">
       <label  class="col-sm-3 control-label " >素材权限</label>
        <div class="col-sm-8">
           <?php $_result=get_dict('sys_yes_no');if(is_array($_result) || $_result instanceof \think\Collection || $_result instanceof \think\Paginator): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dict): $mod = ($i % 2 );++$i;?>
            <div class="radio-box">
                <input type="radio" id="resource_<?php echo htmlentities((string) $dict['dict_value']); ?>" name="resource" value="<?php echo htmlentities((string) $dict['dict_value']); ?>" <?php echo $dict['dict_value']==$model['resource'] ? "checked"  :  ""; ?>>
                <label for="resource_<?php echo htmlentities((string) $dict['dict_value']); ?>" ><?php echo htmlentities((string) $dict['dict_label']); ?></label>
            </div>
            <?php endforeach; endif; else: echo "" ;endif; ?>
        </div>
    </div>
    <div class="form-group">
       <label  class="col-sm-3 control-label " >到期时间</label>
        <div class="col-sm-8">
           <input  type="text" id="expire_time" class="form-control time-input" placeholder="请输入到期时间" name="expire_time" value="<?php echo htmlentities((string) get_date($model['expire_time'])); ?>"  />
        </div>
    </div>
    <div class="form-group">
    ...
    <div class="form-group">
       <label  class="col-sm-3 control-label " >状态</label>
        <div class="col-sm-8">
           <?php $_result=get_dict('sys_time_status');if(is_array($_result) || $_result instanceof \think\Collection || $_result instanceof \think\Paginator): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dict): $mod = ($i % 2 );++$i;?>
            <div class="radio-box">
...
<script>
var platform = $("#platform_type_input").val();
if ($.common.isNotEmpty(platform)){
    let platformArr = platform.split(",");
    $.each(platformArr,function (key,value) {
        $(".platform_type_"+value).iCheck('check');
    })
}
</script>
