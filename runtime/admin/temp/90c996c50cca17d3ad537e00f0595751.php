...
<div class="wrapper wrapper-content animated fadeInRight ibox-content">
    <form class="form-horizontal m" id="hr-form" data-ref="translateEngine">
        <input  type="hidden" name="id" value="<?php echo htmlentities((string) $model['id']); ?>"/>
            <div class="form-group">
       <label  class="col-sm-3 control-label is-required" >引擎名称</label>
        <div class="col-sm-8">
           <input  type="text" id="engine_name" class="form-control " placeholder="请输入引擎名称" name="engine_name" value="<?php echo htmlentities((string) $model['engine_name']); ?>" required />
        </div>
    </div>
    <div class="form-group">
       <label  class="col-sm-3 control-label is-required" >引擎编码</label>
        <div class="col-sm-8">
           <input  type="text" id="engine_code" class="form-control " placeholder="请输入引擎编码" name="engine_code" value="<?php echo htmlentities((string) $model['engine_code']); ?>" required />
        </div>
    </div>
    <div class="form-group">
       <label  class="col-sm-3 control-label is-required" >引擎类型</label>
        <div class="col-sm-8">
           <?php $_result=get_dict('sys_trans_type');if(is_array($_result) || $_result instanceof \think\Collection || $_result instanceof \think\Paginator): $i = 0; $__LIST__ = $_result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$dict): $mod = ($i % 2 );++$i;?>
<div class="radio-box">
    <input type="radio" id="engine_type_<?php echo htmlentities((string) $dict['dict_value']); ?>" name="engine_type" value="<?php echo htmlentities((string) $dict['dict_value']); ?>" <?php echo $dict['dict_value']==$model['engine_type'] ? "checked"  :  ""; ?>>
    <label for="engine_type_<?php echo htmlentities((string) $dict['dict_value']); ?>" ><?php echo htmlentities((string) $dict['dict_label']); ?></label>
</div>
<?php endforeach; endif; else: echo "" ;endif; ?>
        </div>
    </div>
    <div class="form-group">
       <label  class="col-sm-3 control-label is-required" >引擎排序</label>
        <div class="col-sm-8">
           <input  type="text" id="engine_sort" class="form-control " placeholder="请输入引擎排序" name="engine_sort" value="<?php echo htmlentities((string) $model['engine_sort']); ?>" required />
        </div>
    </div>
    <div class="form-group">
