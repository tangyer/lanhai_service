    ...
    if ($(".time-input").length > 0) {
        layui.use('laydate', function () {
            ...
            $(".time-input").each(function (index, item) {
                var time = $(item);
                // 控制控件外观
                var type = time.data("type") || 'date';
                // 控制回显格式
                var format = time.data("format") || 'yyyy-MM-dd';
                // 控制日期控件按钮
                var buttons = time.data("btn") || 'clear|now|confirm', newBtnArr = [];
                // 日期控件选择完成后回调处理
                var callback = time.data("callback") || {};
                if (buttons) {
...
// select 下拉数据加载
    $.fn.ajaxSelect = function(option){
        ...
        if ($.isEmptyObject(option)){
            option = {};
            option.url = _this.data("url") || "";
            option.value = _this.data("value") || "";
            option.field= _this.data("field") || "";
            option.key = _this.data("key") || "id";
            option.defaultText = _this.data("text") || "请选择";
            option.showDefault = _this.data("show") !== "0";
        }
        option = $.extend(def,option);
        var html = "";
        if (option.showDefault === true) {
            html = '<option value="'+option.defaultVal+'">'+option.defaultText+'</option>';
        }
        if (option.url !== "") {
            $.getJSON(option.url,option.params,function(res){
                // var data = res.data
                res.hasOwnProperty('data') ? res = res.data : res;
                $.each(res,function(k,v){
                    let selected = "";
                    if (typeof(option.value) == "object") {
                        if ($.inArray(v[option.key], option.value)) selected="selected";else selected="";
                    } else {
                        if(v[option.key] == option.value || v[option.field] == option.value) selected="selected";else selected="";
                    }
                    html += '<option value="'+v[option.key]+'" '+selected+'>'+v[option.field]+'</option>';
                });
                _this.html(html);
            });
        }else{
            ...
            $.each(data,function(k,v){
                if(!isNaN(v)) k = v;
                let selected = "";
                if(k === option.value) selected="selected";else selected="";
